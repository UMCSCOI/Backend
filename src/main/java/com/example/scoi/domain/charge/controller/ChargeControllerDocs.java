...
public interface ChargeControllerDocs {
    ...
    ApiResponse<ChargeResDTO.ChargeKrw> chargeKrw(@AuthenticationPrincipal AuthUser user, @RequestBody ChargeReqDTO.ChargeKrw dto);

    @Operation(
            summary = "특정 주문 확인하기 API By 김주헌",
            description = "특정 주문을 UUID로 스냅샷 형태로 확인합니다. 주문 체결 알림은 웹소켓 이용해서 실시간 추적, 체결 되면 FCM 토큰으로 알림이 갑니다."
    )
    ApiResponse<String> getOrders(@AuthenticationPrincipal AuthUser user, @RequestBody ChargeReqDTO.GetOrder dto);
}
